...
export default function MatchesPage({ onLikeBack, onPass }) {
    ...
    const handleWithdraw = async (targetId) => {
        ...
        try {
            ...
            if (!res.ok) setYouLiked((prev) => [...prev, dev]);
        } catch {
    ...
    return (
        <div
            className="min-h-screen bg-zinc-950 text-zinc-200 overflow-y-auto"
        ...
        >
                ...
                {celebration.show && celebration.dev && (
                    ...
                    >
                        <motion.div
                            ...
                            className="relative w-full max-w-sm mx-4 bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden"
                        >
                            <div className="relative p-8 flex flex-col items-center text-center gap-5">
                                <div className="w-14 h-14 rounded-2xl bg-emerald-950 border border-emerald-900 flex items-center justify-center">
                                    <MatchIcon />
                                </div>
                                <div>
                                    <p className="text-[10px] text-emerald-500 tracking-[.2em] uppercase font-mono mb-2">// mutual match</p>
                                    ...
                                    <p className="text-sm text-zinc-400 leading-relaxed">
                                        ...
                                        liked each other. Messaging is now unlocked.
                                    </p>
                                </div>
                                <div className="flex items-center gap-3">
                                    <div className="w-12 h-12 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-sm font-bold text-zinc-300"
                                        style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                                    ...
                                    <div className="w-12 h-12 rounded-full overflow-hidden border border-zinc-700 flex items-center justify-center bg-zinc-800">
                                ...
                                <div className="flex gap-2 w-full">
                                    <button
                                        onClick={() => { setCelebration({ show: false, dev: null }); navigate(`/messages/${celebration.dev.match_id}`); }}
                                        className="flex-1 py-2.5 text-[11px] font-semibold tracking-widest rounded-lg border border-emerald-800 bg-emerald-950 text-emerald-300 hover:bg-emerald-900 hover:border-emerald-700 transition-all"
                                    >
                                        ./open-chat
                                    </button>
                                    <button
                                        onClick={() => { setCelebration({ show: false, dev: null }); setActiveTab("matched"); }}
                                        className="flex-1 py-2.5 text-[11px] font-semibold tracking-widest rounded-lg border border-zinc-800 text-zinc-500 hover:border-zinc-700 hover:text-zinc-300 transition-all"
                                    >
                                        view matches
                                    </button>
                                </div>
            ...
            <div className="absolute top-6 right-6 md:right-12 z-40 pt-16">
                <button
                    ...
                    className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold tracking-widest px-5 py-2.5 rounded-lg transition-all"
                >
                    <PlusIcon />
                    ./discover-devs
                </button>
            ...
            <div className="max-w-4xl mx-auto px-6 md:px-12 py-10 flex flex-col gap-8 mt-12">
                ...
                <motion.div variants={fadeUp} initial="hidden" animate="show" custom={0}>
                    ...
                    <h1 className="font-bold tracking-tight leading-[1.05]"
                        style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "clamp(26px,4vw,40px)", letterSpacing: "-0.03em" }}>
                    ...
                    <div className="flex gap-1 bg-zinc-900/60 border border-zinc-800 rounded-xl p-1">
                        {TABS.map(({ key, label, prefix, prefixColor }) => (
                            <button
                                ...
                                className={`relative flex-1 flex flex-col items-center gap-0.5 py-2.5 px-3 rounded-lg text-[11px] font-semibold tracking-wider transition-all ${activeTab === key ? "bg-zinc-800 text-zinc-100" : "text-zinc-600 hover:text-zinc-400"
                                    }`}
                            >
                                ...
                                <span>{label}</span>
                                <span className={`absolute top-2 right-2 text-[10px] px-1.5 py-0.5 rounded-full font-mono ${key === "liked-you" ? "bg-indigo-950 text-indigo-400 border border-indigo-900" :
                                        key === "matched" ? "bg-emerald-950 text-emerald-400 border border-emerald-900" :
                    ...
                    >
                        ...
                        {!loading && activeData.map((dev, i) => {
                            if (activeTab === "liked-you") return (
                                <LikeCard key={dev.userid} dev={dev} index={i}
            ...
            <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-4 flex flex-col gap-3">
                <div className="flex gap-3 items-start">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-indigo-400 font-medium tracking-wide mt-0.5">{dev.devType || dev.role}</p>
                ...
                {dev.goals?.length > 0 && (
                    <div className="bg-zinc-800/40 border border-zinc-700/50 rounded-lg px-3 py-2.5">
                        <p className="text-[9px] text-indigo-400 tracking-[.15em] uppercase mb-1.5 font-semibold">// looking for</p>
                        <div className="flex flex-wrap gap-1.5">
                            {dev.goals.slice(0, 3).map((g) => (
                                <span key={g} className="text-[10px] px-2 py-0.5 rounded bg-indigo-950 text-indigo-300 border border-indigo-900 font-medium">{g}</span>
                            ))}
                        </div>
                    </div>
                )}
                ...
                {dev.bio && <p className="text-[11px] text-zinc-500 leading-relaxed line-clamp-2">{dev.bio}</p>}
                <div className="flex gap-2 pt-1">
                    <button onClick={onViewProfile}
                        className="px-3 py-2 text-[11px] font-semibold tracking-widest rounded-lg border border-zinc-800 text-zinc-500 hover:border-zinc-700 hover:text-zinc-300 transition-all">
                        ./view-profile
                    </button>
                    <button onClick={onLikeBack}
                        className="flex-1 py-2 text-[11px] font-semibold tracking-widest rounded-lg border border-indigo-800 bg-indigo-600 text-white hover:bg-indigo-500 hover:border-indigo-500 transition-all">
                        like back →
                    </button>
                    <button onClick={onPass}
                        className="px-3 py-2 text-[11px] text-zinc-600 rounded-lg border border-zinc-800 hover:border-zinc-700 hover:text-zinc-400 transition-all">
                        pass
                    </button>
                </div>
            </div>
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="show" custom={index}>
            <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-4 flex flex-col gap-3">
                <div className="flex gap-3 items-start">
                    ...
                    <span className="shrink-0 text-[10px] px-2 py-1 rounded-md bg-emerald-950 text-emerald-500 border border-emerald-900 font-mono tracking-wider">
                        matched
                    </span>
                </div>
                {dev.stack?.length > 0 && (
                ...
                <div className="flex gap-2 pt-1">
                    ...
                    <button onClick={onOpenChat}
                        className="flex-1 py-2 text-[11px] font-semibold tracking-widest rounded-lg border border-emerald-900 bg-emerald-950 text-emerald-300 hover:bg-emerald-900 hover:border-emerald-800 transition-all">
                        ./open-chat
                    </button>
                </div>
            </div>
    ...
    return (
        <motion.div variants={fadeUp} initial="hidden" animate="show" custom={index}>
            <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-4 flex flex-col gap-3">
                ...
                <div className="flex gap-2 pt-1">
                    <button onClick={onViewProfile}
                        className="flex-1 px-3 py-2 text-[11px] font-semibold tracking-widest rounded-lg border border-zinc-800 text-zinc-500 hover:border-zinc-700 hover:text-zinc-300 transition-all">
                        ./view-profile
                    </button>
                    {!confirming ? (
                        <button onClick={() => setConfirming(true)}
                            className="px-3 py-2 text-[11px] font-semibold tracking-widest rounded-lg border border-zinc-800 text-zinc-600 hover:border-red-900 hover:text-red-400 hover:bg-red-950/40 transition-all">
                            withdraw
                        </button>
                    ) : (
                        <div className="flex gap-1.5">
                            <button onClick={onWithdraw}
                                className="px-3 py-2 text-[11px] font-semibold tracking-widest rounded-lg border border-red-900 bg-red-950/60 text-red-400 hover:bg-red-950 transition-all">
                                confirm
                            </button>
...
function EmptyState({ tab, onDiscover }) {
    ...
    return (
        <div className="py-16 text-center border border-dashed border-zinc-800 rounded-2xl flex flex-col items-center gap-3">
            <p className="text-[11px] text-zinc-600 tracking-widest">{text}</p>
            <p className="text-[11px] text-zinc-700">{sub}</p>
            {tab === "you-liked" && (
                <button onClick={onDiscover}
                    className="mt-2 text-[11px] font-semibold tracking-widest text-indigo-400 border border-indigo-900 bg-indigo-950/40 hover:bg-indigo-950 px-5 py-2 rounded-lg transition-all">
                    ./discover-devs
                </button>
            )}
        </div>
    ...
    return (
        <div className="w-10 h-10 rounded-full overflow-hidden border border-zinc-800 shrink-0">
            {dev.avatarUrl ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-xs font-bold text-zinc-500 bg-zinc-800"
                    style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
...
function AnonTag() {
    return (
        <span className="text-[10px] px-2 py-0.5 rounded bg-zinc-800 text-zinc-500 border border-zinc-700 font-mono">photo hidden</span>
    );
}
